...
public class MainWindowController implements Initializable {
	...
	public void initialize(final URL location, final ResourceBundle resources) {
		...
		// next to it
		channelCustomInput.focusedProperty().addListener(
		...
		streamServiceSelection.getItems().addAll(
				StreamServiceManager.getStreamServices());
		...
		streamServiceSelection
				.setCellFactory(new Callback<ListView<StreamService>, ListCell<StreamService>>() {
					@Override
					public ListCell<StreamService> call(
							final ListView<StreamService> param) {
		...
		qualitySelection
				.setCellFactory(new Callback<ListView<Quality>, ListCell<Quality>>() {
					@Override
	...
	protected void onLiveClicked(final ActionEvent event) {
		// Start stream
		final StreamService selectedStreamService = streamServiceSelection
				.getValue();
		...
		if (channelCustom.isSelected()) {
			try {
				StreamServiceManager.startStream(selectedStreamService,
						channelCustomInput.getText(), quality);
